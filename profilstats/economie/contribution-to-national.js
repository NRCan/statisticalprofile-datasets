...
(function () {
  var draw_chart1 = function (event) {
    d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatsProfile/Economy/Contribution-to-national/After_2000_StatsCan-Nominal_and_Real-GDP-EN_FR.csv")
        .then(function (data) {
            console.log(data);
    ...
    function lineChart1(data, Industry) {
        ...
        const svg = d3.select("#contribution-to-national-chart")
            .append( "h2" )
                    .text( "Nominal GDP (current $)" )
                    .append("svg")
                    .attr("role", "img")
                    .attr( "aria-label", "Nominal GDP (current $) chart - Data table in following section" )
        ...
        const yScale = d3.scaleLinear()
            .domain([0, d3.max(data, d => +d['Nominal'])])
            .range([height, 0]).nice();
        ...
        svg.append("rect")
            ...
            .on("mousemove", function (event) {
                ...
                tooltipCircle
                    ...
                    .attr("cy", yScale(+d['Nominal']));

                tooltip
                    .style("left", `${event.pageX + 10}px`)
                    .style("top", `${event.pageY - 300}px`)
                    .html(`Year: <b>${d.Year}</b><br>Amount($): <b>${Number.parseInt( d['Nominal'] ).toLocaleString( 'en-CA' )}</b>`);
            })
    ...
    function lineChart2(data, Industry) {
        ...
        const svg = d3.select("#contribution-to-national-chart")  // Append SVG to body or any other container you'd prefer
            .append( "h2" )
                    .text( "Real GDP (constant 2012 $)" )
                    .append("svg")
                    .attr("role", "img")
                    .attr( "aria-label", "Real GDP (constant 2012 $) chart - Data table in following section" )
            .attr("preserveAspectRatio", "xMinYMin meet")
            .attr("viewBox", "0 0 " + (width + margin.left + margin.right) + " " + (height + margin.top + margin.bottom))
            .append("g")
            .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
        ...
        svg.append("rect")
            ...
            .on("mousemove", function (event) {
                ...
                tooltipCircle
                    .attr("cx", xScale(+d.Year))
                    .attr("cy", yScale(+d['Real']));

                tooltip
                    .style("left", `${event.pageX + 10}px`)
                    .style("top", `${event.pageY - 300}px`)
                    .html(`Year: <b>${d.Year}</b><br>Amount($) <b>${Number.parseInt( d['Real'] ).toLocaleString( 'en-CA' )}</b>`);
            })
            .on("mouseout", function () {
                tooltip.style("display", "none");
                tooltipCircle.style("display", "none");
