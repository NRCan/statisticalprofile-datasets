// Capital expenditures and repair expenditures chart
    ...
    (function () {
      var draw_chart1 = function (event) {
        // Read the CSV file
        d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatsProfile/Investment/After_2000_StatsCan-Expenditures-EN_FR.csv")
            .then(function (data) {
                console.log(data);

                // Extract unique values from NAICS column
                let uniqueNaics = [...new Set(data.map(item => item.NAICS))];

                // Get the dropdown element
                let dropdown = d3.select("#NAICS-select");
                ...
                dropdown.on("change", function () {
                    ...
                    console.log("Selected NAICS: ", selectedValue);
                    d3.selectAll('#charts svg, #charts h2').remove()

                    lineChart(data, selectedValue, "Capital expenditures", '#335075')
                    lineChart(data, selectedValue, "Repair expenditures", '#1b6c1c')
                });

                console.log(dropdown.property("value"))
                var selected_NAICS = dropdown.property("value")
                lineChart(data, selected_NAICS, "Capital expenditures", '#335075')
                lineChart(data, selected_NAICS, "Repair expenditures", '#1b6c1c')

            });


        function lineChart(data, NAICS, attribute, color) {
            data = data.filter(d => (d.NAICS == NAICS) && (d['Capital-and-repair-expenditures'] == attribute))
            console.log(data)
            ...
            const svg = d3.select("#charts")
				...
				.append("svg")
				.attr("role", "img")
				.attr( "aria-label", "Capital expenditures and repair expenditures chart - Data table in following section" )
            ...
            svg.append("rect")
                ...
                .on("mousemove", function (event) {
                    ...
                    tooltip
                        ...
                        .html(`Year: <b>${d.Year}</b><br>Amount($): <b>${Number.parseInt( d['Value'] ).toLocaleString( 'en-CA' )}</b>`);
