...
(function () {
    var draw_chart2 = function (event) {
      // Read the CSV file
      d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Employment/StatProfile_Employment_SNA_EN.csv")
          .then(function (data) {
              console.log(data);
              ...
              let dropdown = d3.select("#sna_jurisdiction-select");
              ...
              let uniqueSector = [...new Set(data.map(item => item.Sector))];
                  ...
                  // Get the dropdown element
                  let dropdownSector = d3.select("#sna_sector-select");
              ...
              dropdown.on("change", function () {
                  ...
                  d3.selectAll('#sna-chart svg').remove()
      ...
      function lineChart(data, Jurisdiction, Sector) {
          ...
          const svg = d3.select("#sna-chart")  // Append SVG to body or any other container you'd prefer
              .append("svg")
          ...
          const yScale = d3.scaleLinear()
              .domain([0, d3.max(data, d => +d['Number of jobs'])])
              .range([height, 0]).nice();
          ...
          svg.append("rect")
              ...
              .on("mousemove", function (event) {
                  ...
                  tooltipCircle
                      ...
                      .attr("cy", yScale(+d['Number of jobs']));
  
                  tooltip
                      .style("left", `${event.pageX + 10}px`)
                      .style("top", `${event.pageY - 300}px`)
                      .html(`Year: <b>${d.Year}</b><br>Number of jobs: <b>${Number.parseInt( d['Number of jobs'] ).toLocaleString( 'en-CA' )}</b>`);
              })
