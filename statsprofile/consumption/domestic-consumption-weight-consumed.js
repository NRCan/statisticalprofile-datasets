// Weight consumed chart

  (function () {
    var draw_chart2 = function (event) {
        // Read the CSV file
        d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Consumption/StatProfile_Trade_comsumption_EN.csv")
            .then(function (data) {
                console.log(data);

                // Extract unique values from NAICS column
                let unique = ["Printing and writing paper (tonnes)", "Wood pulp (tonnes)", "Newsprint (tonnes)"];

                // Get the dropdown element
                let dropdown = d3.select("#weight_consumed-select");
                ...
                dropdown.on("change", function () {
                    ...
                    d3.selectAll('#weight_consumed-chart svg').remove()
            ...
            function lineChart(data, Product) {
                data = data.filter(d => (d['Commodity Group'] == Product))
                console.log(data)
            ...
            const svg = d3.select("#weight_consumed-chart")  // Append SVG to body or any other container you'd prefer
                .append("svg")
            ...
            const yScale = d3.scaleLinear()
                ...
                .domain([d3.min(data, d => Math.min(0, d['Domestic consumption'])), d3.max(data, d => Math.max(0, d['Domestic consumption']))])
                .range([height, 0]).nice();
            ...
            svg.append("rect")
                ...
                .on("mousemove", function (event) {
                    ...
                    tooltipCircle
                        ...
                        .attr("cy", yScale(+d['Domestic consumption']));

                    tooltip
                        .style("left", `${event.pageX + 10}px`)
                        .style("top", `${event.pageY - 300}px`)
                        .html(`Year: <b>${d.Year}</b><br>Consumption: <b>${Number.parseInt( d['Domestic consumption'] ).toLocaleString( 'en-CA' )}</b>`);
                })
