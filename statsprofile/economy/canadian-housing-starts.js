//Canadian housing starts chart 

(function () {
    var draw_chart1 = function (event) {
      d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Economy/StatProfile_Economy_housing_EN.csv")
          .then(function (data) {
              console.log(data);
              ...
              let dropdown = d3.select("#canadian_housing_starts-select");
              ...
              dropdown.on("change", function () {
                  ...
                  d3.selectAll('#canadian_housing_starts-chart svg').remove()
    ...
    //   }
      ...
      function lineChart(data, Jurisdiction) {
          ...
          const svg = d3.select("#canadian_housing_starts-chart")  // Append SVG to body or any other container you'd prefer
              .append("svg")
          ...
          const yScale = d3.scaleLinear()
              .domain([0, d3.max(data, d => +d['Housing starts (total units)'])])
              .range([height, 0]).nice();
          ...
          const line = d3.line()
              .x(d => xScale(+d.Year))
              .y(d => yScale(+d['Housing starts (total units)']));
          ...
          svg.append("rect")
              ...
              .on("mousemove", function (event) {
                  ...
                  tooltipCircle
                      ...
                      .attr("cy", yScale(+d['Housing starts (total units)']));

                  tooltip
                      .style("left", `${event.pageX + 10}px`)
                      .style("top", `${event.pageY - 300}px`)
                      .html(`Year: <b>${d.Year}</b><br>Number of houses: <b>${Number.parseInt( d['Housing starts (total units)'] ).toLocaleString( 'en-CA' )}</b>`);
              })
