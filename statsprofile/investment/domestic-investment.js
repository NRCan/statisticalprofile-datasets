// Capital expenditures and repair expenditures chart

    (function () {
      var draw_chart1 = function (event) {
        // Read the CSV file
        d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Investment/StatProfile_Investment_expend_EN.csv")
            .then(function (data) {
                console.log(data);

                // Extract unique values from Measure column
                let uniqueMeasure = [...new Set(data.map(item => item.Measure))];

                // Get the dropdown element
                let dropdown = d3.select("#measure-select");
                ...
                dropdown.on("change", function () {
                    let selectedMeasure = d3.select(this).property("value");
                    console.log("Selected Measure: ", selectedMeasure);
                    d3.selectAll('#charts svg, #charts h2').remove()

                    lineChart(data, selectedMeasure, "Capital expenditures", '#335075')
                    lineChart(data, selectedMeasure, "Repair expenditures", '#1b6c1c')
                });

                console.log(dropdown.property("value"))
                var selectedMeasure = dropdown.property("value")
                lineChart(data, selectedMeasure, "Capital expenditures", '#335075')
                lineChart(data, selectedMeasure, "Repair expenditures", '#1b6c1c')
        ...
        function lineChart(data, measure, attribute, color) {
            data = data.filter(d => (d.Measure == measure) && (d['Capital and repair expenditures'] == attribute))
            console.log(data)
            ...
            const svg = d3.select("#charts")
                .append( "h2" )
                .text( attribute + " in Canada's forest sector")
				.append("svg")
				.attr("role", "img")
				.attr( "aria-label", "Capital expenditures and repair expenditures charts - Data table in following section" )
            ...
            svg.append("text")
                ...
                .attr("text-anchor", "middle")
                .attr("font-size", "13px")
                .text("Expenditures (1,000 dollars)");
            ...
            svg.append("rect")
                ...
                .on("mousemove", function (event) {
                    ...
                    tooltip
                        ...
                        .html(`Year: <b>${d.Year}</b><br>Amount: <b>${Number.parseInt( d['Value (1,000 dollars)'] ).toLocaleString( 'en-CA' )}</b>`);
