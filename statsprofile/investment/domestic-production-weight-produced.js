...
.then(function (data) {
  ...
  let unique = ["Newsprint", "Printing and writing paper", "Wood pulp"];

  // Get the dropdown element
  let dropdown = d3.select("#weight_produced-select");
  ...
  dropdown.on("change", function () {
      ...
      d3.selectAll('#weight_produced-chart svg').remove()
...
function lineChart(data, Product) {
  data = data.filter(d => (d['Forest Product'] == Product) && (d['Unit of measure'] == "Tonnes" || "tonnes"))
  console.log(data)
...
// Append SVG to body
const svg = d3.select("#weight_produced-chart")  
  .append("svg")
...
svg.append("rect")
  ...
  .on("mousemove", function (event) {
      ...
      tooltip
          ...
          .html(`Year: <b>${d.Year}</b><br>Weight(t): <b>${Number.parseInt( d['Value'] ).toLocaleString( 'en-CA' )}</b>`);
  })
