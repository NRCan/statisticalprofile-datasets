...
(function () {
    var draw_chart1 = function (event) {
      // Read the CSV file
      d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Trade/StatProfile_Trade_balance_value_EN.csv")
          .then(function (data) {
              console.log(data);
            ...
            //   let dropdown = d3.select("#balance_of_trade_jurisdiction-select");
            ...
            //       .attr("value", d => d); 
              ...
              let uniqueCommodity = [...new Set(data.map(item => item['Commodity Group']))];
  
                  // Get the dropdown element
                  let dropdownCommodity = d3.select("#balance_of_trade_commodity-select");
  
                  // Append unique values as options
                  dropdownCommodity.selectAll("option.data-option")
                      .data(uniqueCommodity)
                      .enter()
                      .append("option")
                      .classed("data-option", true)  // add a class for later reference
                      .text(d => d)
            ...
            //       let selectedCommodity = dropdownCommodity.property("value");
            //       d3.selectAll('#balance_of_trade-chart svg').remove()
            //       lineChart(data, selectedJurisdiction, selectedCommodity)
            //   });
              ...
              dropdownCommodity.on("change", function () {
                //   let selectedJurisdiction = dropdown.property("value");
                  let selectedCommodity = dropdownCommodity.property("value");
                  d3.selectAll('#balance_of_trade-chart svg').remove()
                  lineChart(data, selectedCommodity)
              });
  
            //   var selectedJurisdiction = dropdown.property("value")
              var selectedCommodity = dropdownCommodity.property("value")
              lineChart(data, selectedCommodity);
  
          });
  
  
      function lineChart(data, Commodity) {
         
          data = data.filter(d => (d.Jurisdiction == "Canada") && (d['Commodity Group'] == Commodity))
          ...
          const svg = d3.select("#balance_of_trade-chart")  // Append SVG to body or any other container you'd prefer
              .append("svg")
          ...
          const yScale = d3.scaleLinear()
            //   .domain([0, d3.max(data, d => +d['Balance of trade (dollars)'])])
              .domain([d3.min(data, d => Math.min(0, d['Balance of trade (dollars)'])), d3.max(data, d => Math.max(0, d['Balance of trade (dollars)']))])
              .range([height, 0]).nice();
          ...
          svg.append("text")
              ...
              .attr("text-anchor", "middle")
              .attr("font-size", "13px")
              .text("Balance of trade (dollars)");
  
          // Line generator
          const line = d3.line()
              .x(d => xScale(+d.Year))
              .y(d => yScale(+d['Balance of trade (dollars)']));
          ...
          svg.append("rect")
              ...
              .on("mousemove", function (event) {
                  ...
                  tooltipCircle
                      ...
                      .attr("cy", yScale(+d['Balance of trade (dollars)']));
  
                  tooltip
                      .style("left", `${event.pageX + 10}px`)
                      .style("top", `${event.pageY - 300}px`)
                      .html(`Year: <b>${d.Year}</b><br>Balance of trade: <b>${Number.parseInt( d['Balance of trade (dollars)'] ).toLocaleString( 'en-CA' )}</b>`);
              })
