// Maple products trade quantity chart

(function () {
    var draw_chart2 = function (event) {
      // Read the CSV file
      d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Trade/StatProfile_Trade_Maple_EN.csv")
          .then(function (data) {
              console.log(data);
  
              // Extract unique values from Commodity column
              let unique = [...new Set(data.map(item => item.Commodity))];
  
              // Get the dropdown element
              let dropdown = d3.select("#maple_products_quantity_commodity-select");
              ...
              let uniqueType = [...new Set(data.map(item => item['Trade type']))];
  
                  // Get the dropdown element
                  let dropdownType = d3.select("#maple_products_quantity_type-select");
  
                  // Append unique values as options
                  dropdownType.selectAll("option.data-option")
                      .data(uniqueType)
                      .enter()
                      .append("option")
                      .classed("data-option", true)  // add a class for later reference
              ...
              dropdown.on("change", function () {
                  let selectedCommodity = dropdown.property("value");
                  let selectedType = dropdownType.property("value");
                  d3.selectAll('#maple_products_quantity-chart svg').remove()
                  lineChart(data, selectedCommodity, selectedType)
              });
              ...
              var selectedCommodity = dropdown.property("value")
              var selectedType = dropdownType.property("value")
              lineChart(data, selectedCommodity, selectedType);
  
          });
  
  
      function lineChart(data, Commodity, Type) {
          data = data.filter(d => (d.Commodity == Commodity) && (d['Trade type'] == Type))
          ...
          const svg = d3.select("#maple_products_quantity-chart")  // Append SVG to body or any other container you'd prefer
              .append("svg")
          ...
          svg.append("rect")
              ...
              .on("mousemove", function (event) {
                  ...
                  tooltipCircle
                      ...
                      .attr("cy", yScale(+d['Quantity']));
  
                  tooltip
                      .style("left", `${event.pageX + 10}px`)
                      .style("top", `${event.pageY - 300}px`)
                      .html(`Year: <b>${d.Year}</b><br>Quantity: <b>${Number.parseInt( d['Quantity'] ).toLocaleString( 'en-CA' )}</b>`);
              })
