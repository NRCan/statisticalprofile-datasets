...
(function () {
    var draw_chart2 = function (event) {
      // Read the CSV file
      d3.csv("https://raw.githubusercontent.com/BenoitJPage/Datasets/main/StatProfile-2024_data/Trade/StatProfile_Trade_ExportMarkets_EN.csv")
          .then(function (data) {
              console.log(data);
            ...
            //   let dropdown = d3.select("#export_markets_jurisdiction-select");
            ...
            //       .attr("value", d => d); 
              ...
              let uniqueMarket = [...new Set(data.map(item => item['Export market']))];
  
                  // Get the dropdown element
                  let dropdownMarket = d3.select("#export_markets_market-select");
  
                  // Append unique values as options
                  dropdownMarket.selectAll("option.data-option")
                      .data(uniqueMarket)
                      .enter()
                      .append("option")
                      .classed("data-option", true)  // add a class for later reference
            ...
            //       let selectedMarket = dropdownMarket.property("value");
            //       d3.selectAll('#export_markets-chart svg').remove()
            //       lineChart(data, selectedJurisdiction, selectedMarket)
            //   });
  
              dropdownMarket.on("change", function () {
                //   let selectedJurisdiction = dropdown.property("value");
                  let selectedMarket = dropdownMarket.property("value");
                  d3.selectAll('#export_markets-chart svg').remove()
                  lineChart(data, selectedMarket)
              });
  
            //   var selectedJurisdiction = dropdown.property("value")
              var selectedMarket = dropdownMarket.property("value")
              lineChart(data, selectedMarket);
  
          });
  
  
      function lineChart(data, Market) {
          
          data = data.filter(d => (d.Jurisdiction == "Canada") && (d['Export market'] == Market))
          ...
          const svg = d3.select("#export_markets-chart")  // Append SVG to body or any other container you'd prefer
              .append("svg")
          ...
          svg.append("rect")
              ...
              .on("mousemove", function (event) {
                  ...
                  tooltip
                      ...
                      .html(`Year: <b>${d.Year}</b><br>Export market: <b>${Number.parseInt( d['Value (dollars)'] ).toLocaleString( 'en-CA' )}</b>`);
              })
